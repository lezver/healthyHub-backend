...
const { updateBreakfasts } = require("../../models/day/day");
...
const updateBreakfast = async (req, res, next) => {
    try {
        ...
        const newMeals = await updateBreakfasts(req, req.body);

        if (newMeals === null) {
            return res.status(400).json({ message: "Invalid request data", details: error.details });
        }
        return res.status(200).json({ message: "Breckfast is done!" });
    } catch (error) {
        next(error);
    }
};

module.exports = updateBreakfast;